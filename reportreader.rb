...
#    INPUTS    #
	...
	# parsed from the command-line by OptionParser.
 	...
 	optparse = OptionParser.new { |opts|
   	...
   	#options[:sort] = nil
   	#opts.on( '-s', '--sort PARAM', 'sort by PARAM: comment, roles, hostname, aliases, ip, jpark') do|sortParam|
...
#    OUTPUTS    #
  ...
  def print_host_changes( anArr )
    ...
    ob2.each {|v| 
			...
			puts(
				%Q[\nReport(#{v[:report_format].to_s}): #{v[:host]}  #{v[:time]} Config:#{v[:configuration_version]} Agent:#{puppet_version}  #{v[:status]}  #{string} ]
...
# reportDir = "#{ENV['HOME']}/opt/reports/" if reportDir == nil # Set reportDir if not defined 
...
# Check if file is being executed or loaded as a library
if __FILE__ == $0 then 
  ...
  # Begin menu
	begin 
		...
		print("Reading reports:...")  if options[:verbose]
			...
			#a = read_reports(checkedList, options[:verbose]) if options[:hours] == nil and options[:verbose]
			#a = read_reports(checkedList, options[:hours], options[:verbose]) if options[:hours] and options[:verbose]
		print("done. Loaded #{a.length} records.\n") if options[:verbose]
		
		begin
			if !(options[:changes] or options[:logs])
				puts("What information do you want?")
				print("\n 0: Print host changes.")
				print("\n 1: Print host logs.")
